...
describe("Testing ERC1155",  function () {
  ...
  let ERC721Factory : MyERC1155__factory;
  let test : TEST;
  ...
  before(async function(){
    ...
    ERC721Factory = (await ethers.getContractFactory("MyERC1155"));
    erc1155 = await ERC721Factory.deploy(name, symbol, baseUri);
  ...
  it("check mint() and balanceOf()", async function () {
    ...
    let tx  = await erc1155.mint(owner.address, tokenId, amount);
    await tx.wait();
    ...
    await expect(
      erc1155.connect(hacker).mint(hacker.address, tokenId, amount)
    ).to.be.revertedWith("ERC1155: You are not owner");

    // попытка заминтить токены на нулевой адрес
    await expect(
      erc1155.mint(zeroAddress, tokenId, amount)
  ...
  it("check mintBatch() and balanceOfBatch()", async function () {
    ...
    let tx  = await erc1155.mintBatch(owner.address, tokenIds, [amount, amount]);
    await tx.wait();
    tx  = await erc1155.mintBatch(spender.address, tokenIds, [amount, amount]);
    await tx.wait();
    ...
    // попытка вызвать функцию mint() не от имени owner
    await expect(
      erc1155.connect(hacker).mintBatch(hacker.address, tokenIds, [amount, amount])
    ).to.be.revertedWith("ERC1155: You are not owner");

    // попытка заминтить токены на нулевой адрес
    await expect(
      erc1155.mintBatch(zeroAddress, tokenIds, [amount, amount])
    ).to.be.revertedWith("ERC1155: mint to the zero address");

    // попытка заминтить токены отправив массивы разных размеров
    await expect(
      erc1155.mintBatch(owner.address, tokenIds, [amount, amount, amount])
