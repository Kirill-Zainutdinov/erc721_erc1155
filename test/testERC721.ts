...
describe("Testing ERC721",  function () {
  ...
  before(async function(){
    ...
    erc721 = await ERC721Factory.deploy(name, symbol, baseUri);
  ...
  it("check tokenUri()", async function () {
    ...
    for(let id = 1; id < 6; id++){
      expect(await erc721.tokenURI(id)).equal(baseUri + id.toString());
    }
